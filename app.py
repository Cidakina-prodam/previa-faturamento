...
with st.sidebar:
    ...
    # ── seleção salva — pré-preenche Diretoria/Gerência/Clientes ────────────────
    config_filtros_file = st.file_uploader(
        "💾 Minha seleção de filtros (.json)", type=["json"], key="config_filtros",
        help="Arquivo baixado em um processamento anterior — pré-preenche Diretoria/Gerência/"
             "Clientes com a sua seleção, sem precisar desmarcar tudo de novo a cada CSV novo.",
    )
    config_filtros_salva = {}
    if config_filtros_file is not None:
        try:
            config_filtros_salva = json.loads(config_filtros_file.getvalue().decode("utf-8"))
        except Exception as e:
            st.error(f"Erro ao ler a seleção salva: {e}")

    st.divider()

    # ── diretoria — detectada automaticamente do CSV, se a coluna existir ──
    tem_diretoria = "diretoria" in df_raw.columns
    diretorias_selecionadas = []
    df_pos_diretoria = df_raw
    if tem_diretoria:
        diretorias_existentes = sorted(df_raw["diretoria"].dropna().unique().tolist())
        default_diretoria = [d for d in config_filtros_salva.get("diretoria", diretorias_existentes) if d in diretorias_existentes] \
            if config_filtros_salva else diretorias_existentes
        default_diretoria = default_diretoria or diretorias_existentes
        st.markdown("**Diretoria**")
        st.caption("Detectada automaticamente na coluna `diretoria` do CSV.")
        diretorias_selecionadas = st.multiselect(
            "Diretoria", options=diretorias_existentes, default=default_diretoria,
            label_visibility="collapsed",
        )
        df_pos_diretoria = df_raw[df_raw["diretoria"].isin(diretorias_selecionadas)] if diretorias_selecionadas else df_raw.iloc[0:0]
        st.divider()

    # ── gerência (GDS) — detectada automaticamente do CSV, se a coluna existir ──
    tem_gerencia = "gerencia" in df_raw.columns
    gerencias_selecionadas = []
    df_pos_gerencia = df_pos_diretoria
    if tem_gerencia:
        gerencias_existentes = sorted(df_pos_diretoria["gerencia"].dropna().unique().tolist())
        default_gerencia = [g for g in config_filtros_salva.get("gerencia", gerencias_existentes) if g in gerencias_existentes] \
            if config_filtros_salva else gerencias_existentes
        default_gerencia = default_gerencia or gerencias_existentes
        st.markdown("**Gerência (GDS)**")
        st.caption("Detectada automaticamente na coluna `gerencia` do CSV.")
        gerencias_selecionadas = st.multiselect(
            "Gerência", options=gerencias_existentes, default=default_gerencia,
            label_visibility="collapsed",
        )
        df_pos_gerencia = df_pos_diretoria[df_pos_diretoria["gerencia"].isin(gerencias_selecionadas)] if gerencias_selecionadas else df_pos_diretoria.iloc[0:0]
        st.divider()

    # ── clientes — detectados automaticamente no CSV (após filtro de gerência) ──
    st.markdown("**Clientes**")
    st.caption("Detectados automaticamente no CSV — todos pré-selecionados. Desmarque para excluir.")

    clientes_existentes_no_csv = sorted(
        c for c in df_pos_gerencia["cliente"].dropna().unique().tolist()
        if not (excluir_prodam and c.upper() == "PRODAM")
    )
    default_clientes = [c for c in config_filtros_salva.get("clientes", clientes_existentes_no_csv) if c in clientes_existentes_no_csv] \
        if config_filtros_salva else clientes_existentes_no_csv
    default_clientes = default_clientes or clientes_existentes_no_csv

    if clientes_existentes_no_csv:
        clientes_selecionados = st.multiselect(
            "Clientes", options=clientes_existentes_no_csv, default=default_clientes,
            label_visibility="collapsed",
        )
    else:
    ...
    config_filtros_export = json.dumps({
        "diretoria": diretorias_selecionadas,
        "gerencia": gerencias_selecionadas,
        "clientes": clientes_selecionados,
    }, ensure_ascii=False, indent=2).encode("utf-8")
    st.download_button(
        "💾 Salvar esta seleção",
        data=config_filtros_export,
        file_name=f"config_filtros_{date.today().strftime('%Y%m%d')}.json",
        mime="application/json",
        width="stretch",
        help="Baixe e suba de volta em 'Minha seleção de filtros' no próximo processamento.",
    )
    ...
    st.divider()
    gerar = st.button("⚡ Gerar prévia", type="primary", width="stretch")
...
if tem_diretoria:
    if diretorias_selecionadas:
        df = df[df["diretoria"].isin(diretorias_selecionadas)]
    else:
        st.warning("Nenhuma diretoria selecionada — o relatório ficará vazio.")
        df = df.iloc[0:0]

if tem_gerencia:
    if gerencias_selecionadas:
        df = df[df["gerencia"].isin(gerencias_selecionadas)]
    else:
        st.warning("Nenhuma gerência selecionada — o relatório ficará vazio.")
        df = df.iloc[0:0]
